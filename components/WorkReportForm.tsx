...
const WorkReportForm: React.FC<WorkReportFormProps> = ({
  ...
  // 輔助更新欄位
  const updateField = (updates: Partial<RepairRequest>) => {
    setFormData(prev => ({ ...prev, ...updates }));
  };

  // 判斷是否為「法人協助」的階段
  const isInternal = formData.processingMethod === ProcessingMethod.INTERNAL;
  const isLegal = formData.processingMethod === ProcessingMethod.LEGAL;
  ...
  const Section = ({
    ...
    <div className={`p-8 rounded-[40px] border-2 transition-all duration-500 ${isDone ? 'bg-emerald-50/50 border-emerald-100' :
      ...
      <div className="p-8 bg-white rounded-[40px] border-2 border-indigo-600 shadow-xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.values(ProcessingMethod).map((method) => (
            <button
              key={method}
              onClick={() => updateField({ processingMethod: method })}
              className={`p-6 rounded-[32px] border-2 text-left transition-all ${formData.processingMethod === method
          ...
          <Section title="自行處理詳細報表" isActive={true} isDone={formData.status === RepairStatus.CLOSED} icon={<User size={20} />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                ...
                <input required type="date" className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold border-none outline-none focus:ring-2 focus:ring-indigo-600 transition-all" value={formData.completionDate} onChange={e => updateField({ completionDate: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase">提報人 *</label>
                <input required className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold border-none outline-none focus:ring-2 focus:ring-indigo-600 transition-all" value={formData.reporter} onChange={e => updateField({ reporter: e.target.value })} />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input type="number" className="w-full pl-10 pr-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.amount} onChange={e => updateField({ amount: parseInt(e.target.value) })} />
                </div>
              </div>
              <div className="md:col-span-2 space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase">處理方式與說明 *</label>
                <textarea rows={3} className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.processingDescription} onChange={e => updateField({ processingDescription: e.target.value })} />
              </div>
              <div className="md:col-span-2 space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase">備註</label>
                <input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.remarks} onChange={e => updateField({ remarks: e.target.value })} />
              </div>
              <div className="md:col-span-2">
                <p className="text-[10px] font-black text-slate-400 uppercase mb-3">現場完工照片 ({formData.photoUrls?.length || 0})</p>
                ...
                <button
                  ...
                  className="w-full py-8 border-2 border-dashed border-slate-200 rounded-[32px] text-slate-400 flex flex-col items-center gap-2 hover:bg-white hover:border-indigo-400 hover:text-indigo-600 transition-all group"
                >
                  <Camera size={32} className="group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-black uppercase tracking-widest">點擊或拖放照片上傳</span>
                </button>
              </div>
            </div>

            <div className="flex gap-4 pt-6">
              <button onClick={onCancel} className="px-8 py-5 bg-white border border-slate-200 rounded-[24px] font-black text-slate-400 hover:bg-slate-50 transition-all">返回</button>
              <button
                onClick={() => handleProgressSave(RepairStatus.CONSTRUCTION_DONE)}
                disabled={isSaving}
                className="px-8 py-5 bg-white border border-slate-200 text-indigo-600 font-black rounded-[24px] hover:bg-indigo-50 transition-all"
              >
                暫存並設為完工
              </button>
              <button
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                ...
                <input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.vendor} onChange={e => updateField({ vendor: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400">核定金額 *</label>
                <input type="number" className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.amount} onChange={e => updateField({ amount: parseInt(e.target.value) })} />
              </div>
              <div className="md:col-span-2 space-y-1">
                <label className="text-[10px] font-black text-slate-400">處理方式說明 *</label>
                <textarea rows={2} className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.processingDescription} onChange={e => updateField({ processingDescription: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400">提報人 *</label>
                <input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.reporter} onChange={e => updateField({ reporter: e.target.value })} />
              </div>
              <div className="space-y-1">
          ...
          >
            <div className="space-y-6">
              <label className="flex items-center gap-4 p-5 bg-slate-50 rounded-[24px] border border-slate-100 cursor-pointer hover:bg-white transition-all group">
                ...
                <input type="checkbox" className="hidden" checked={formData.isSignedSent} onChange={e => updateField({ isSignedSent: e.target.checked })} />
              ...
              <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-500 ${formData.isSignedSent ? 'opacity-100' : 'opacity-20 pointer-events-none scale-95'}`}>
                <div className="space-y-1">
                  ...
                  <input type="date" className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.signedSentDate} onChange={e => updateField({ signedSentDate: e.target.value })} />
                </div>
                <div className="space-y-1">
          ...
          >
            <div className="space-y-6">
              <div className="space-y-1">
                ...
                <select className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.paymentEntity} onChange={e => updateField({ paymentEntity: e.target.value })}>
              ...
              <label className="flex items-center gap-4 p-5 bg-slate-50 rounded-[24px] border border-slate-100 cursor-pointer hover:bg-white transition-all group">
                ...
                <input type="checkbox" className="hidden" checked={formData.isWorkFinished} onChange={e => updateField({ isWorkFinished: e.target.checked })} />
                <div className="flex-1">
              ...
              <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-500 ${formData.isWorkFinished ? 'opacity-100' : 'opacity-20 pointer-events-none scale-95'}`}>
                <div className="space-y-1">
                  ...
                  <input type="date" className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.completionDate} onChange={e => updateField({ completionDate: e.target.value })} />
                </div>
                <div className="space-y-1">
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" placeholder="輸入任何特殊狀況..." value={formData.remarks} onChange={e => updateField({ remarks: e.target.value })} />
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="flex items-center gap-4 p-5 bg-white border border-slate-200 rounded-[24px] cursor-pointer hover:bg-slate-50 transition-all">
                  <input type="checkbox" className="w-7 h-7 rounded-lg accent-emerald-600" checked={formData.isInvoiceConfirmed} onChange={e => updateField({ isInvoiceConfirmed: e.target.checked })} />
                  <span className="font-black text-slate-700">發票確認無誤</span>
                </label>
                <label className="flex items-center gap-4 p-5 bg-white border border-slate-200 rounded-[24px] cursor-pointer hover:bg-slate-50 transition-all">
                  <input type="checkbox" className="w-7 h-7 rounded-lg accent-emerald-600" checked={formData.isPaymentSent} onChange={e => updateField({ isPaymentSent: e.target.checked })} />
              ...
              <div className={`space-y-1 transition-all duration-500 ${formData.isPaymentSent ? 'opacity-100' : 'opacity-20 pointer-events-none scale-95'}`}>
                ...
                <input type="date" className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold" value={formData.paymentDate} onChange={e => updateField({ paymentDate: e.target.value })} />
              </div>
