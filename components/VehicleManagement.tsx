...
import { OfficialVehicle, VehicleRecord, Language, Hall } from '../types';
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-[40px] p-10 space-y-8 animate-in zoom-in-95">
            ...
            <form onSubmit={handleAddVehicle} className="grid grid-cols-2 gap-6">
              <div className="space-y-1"><label className="text-xs font-black text-slate-400">會館</label><select className="w-full px-4 py-3 bg-slate-50 rounded-2xl" value={formData.hallName} onChange={e => setFormData({ ...formData, hallName: e.target.value })}>{halls.map(h => <option key={h.id} value={h.name}>{h.name}</option>)}</select></div>
